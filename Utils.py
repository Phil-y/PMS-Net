...
class WeightedBCE(nn.Module):
    ...
    def forward(self, logit_pixel, truth_pixel):
        ...
        # print("logit:{},logit shape:{}".format(logit, logit.shape))
        truth = truth_pixel.view(-1)
        # print("truth:{},truth shape:{}".format(truth, truth.shape))
        assert(logit.shape==truth.shape)

        # logit = torch.sigmoid(logit)
        # # print("logit:{},logit shape:{}".format(logit, logit.shape))
        # loss = F.binary_cross_entropy(logit, truth, reduction='none')
        loss = F.binary_cross_entropy_with_logits(logit, truth, reduction='none') #sigmoid+BCELoss
        # print(loss)
        pos = (truth>0.5).float()
        neg = (truth<0.5).float()
...
class WeightedDiceLoss(nn.Module):
    ...
    def forward(self, logit, truth, smooth=1e-5):#smooth=1e-5
        batch_size = len(logit) #4
...
class WeightedDiceBCE(nn.Module):
    ...
    def forward(self, inputs, targets):
        ...
        # print("dice_BCE_loss", dice_BCE_loss)
